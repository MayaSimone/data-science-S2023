...
- Generally, as the carat of a diamond increases, so to does its price.
  They show a positive correlation.

- Most of the diamonds have a carat value between 0 and 2.
...
``` r
diamonds %>%
  ggplot(aes(cut, carat, color = price))+
  geom_point()
```
...
- Carat seems to be more positively correlated with price than cut is.

- There seems to be minimal differences in price between the ‘ideal’,
  ‘premium’, and ‘very good’ cut diamonds, best shown by the second
  graph

- The ‘fair’ diamonds tend to have a lower price compared to diamonds of
  the same carat.
