...
```
    ...
    ## # A tibble: 1 × 5
    ##    mean    sd       se hi_pi low_pi
    ##   <dbl> <dbl>    <dbl> <dbl>  <dbl>
    ## 1 0.785 0.410 0.000581  3.15   3.14

``` r
df_q3$hi_pi - df_q3$low_pi
```
...
- Does your interval include the true value of $\pi$?
  - Yes, my interval is 3.137409 \< pi \< 3.146511, and the true pi
    value is within that range
- What confidence level did you choose?
  - I chose a 95% confidence interval since it is generally accepted
    that a p-value of less than 0.05 means the results are statistically
    significant\[4\] and the null hypothesis can be rejected (i.e. we
    can make the conclusion that the results are not due to random
    chance)
- Was your sample size $n$ large enough? Why do you say that?
  - I think my sample size (500,000) is large enough, as it was able to
    estimate pi within a range of about 0.0091 with a 95% confidence
    interval. The value generated does not have a specific purpose in
    real life (i.e. medical or aerospace, where a lack of precision can
    cost human lives), so this seems to be an adequate sample size.
...
\[4\] Statistically Significant [Confidence
Intervals](https://www.investopedia.com/terms/c/confidenceinterval.asp#:~:text=In%20general%2C%20a%20p%2Dvalue,within%20a%2095%25%20confidence%20interval.)
