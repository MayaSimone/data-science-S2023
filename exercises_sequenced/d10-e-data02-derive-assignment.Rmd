...
They both result in the same data, but one uses the pipe operator to put the original data set variable first.
...
diamonds %>%
  ...
  summarize(price = mean(price))
```
Commented provides a mean price for all diamonds in the data set while uncommented provides a mean price for each color/clarity group
...
df_q4
```
- cov_depth is small because the differences in depth are small relative to the scale
- cov_diff is massive because the mean difference between depth and calculated_depth is so small
...
- Ideal diamonds have the lowest mean price
- The coeff of variation for price is fairly high (80-101%), so it's likely that other variables effect price
