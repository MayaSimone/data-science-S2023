...
**Observations**:
- The default parameter for position is stacked, so isn't showing the actually 'hwy' values
...
mpg %>%
  ggplot(aes(cty, hwy)) +
  geom_point()
  ##geom_count() will make dots sizes relative to count
```
...
**Observations**:
- 'suv' is the most common class of car, while '2seater' is the least common
- The don't add to the graph, they just reiterate the already seperate/labelled bars
...
**Observations**:
- Rear wheel drive cars are the least common.
- Outside of the SUVs, front wheel drives are more common
...
- `v2` allowed for more observations because it communicates all the same data from `v1` but it instead uses color to show the number of `drv` for each car class. We can draw conclusions about types of drive trains for each class and the overall data set.
...
**Observations**
- Dodge makes the majority of pickups
- Audi, Volkswagen, and Toyota make most compact cars
- Mercury, Lincoln, Land Rover, Jeep only make SUVs
...
mpg %>%
  ggplot(aes(x = cyl)) +
  geom_bar() +
  facet_wrap(~ manufacturer) ##change facet_grid()
```
