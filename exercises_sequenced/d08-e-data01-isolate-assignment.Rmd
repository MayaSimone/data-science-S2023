...
It ends up showing the flights with destination LGA, not BOS because "LGA" was saved to the variable BOS
...
df_q5a <- flights %>% 
  filter(
    dest == "JKF" | dest == "LGA" | dest == "EWR"
    )

df_q5b <- flights %>%
  filter(
    origin == "JKF" | origin == "LGA" | origin == "EWR"
    )
```
