...
ggplot2, purrr, tibble, dplyr, tidyr, stringr, readr, and forcats
...
`install.packages()` is more of a one-time command to add packages to a package library. `library()` can then be used to load these packages into memory each time it is needed (per file)
